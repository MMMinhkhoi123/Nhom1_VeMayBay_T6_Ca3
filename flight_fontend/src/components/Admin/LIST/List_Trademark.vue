<template>
  <div class="list_trademark">
    ...
    >
      <img
        :src="
          'http://localhost:8081/img/trademark/' + item.id + '&3&' + item.img
        "
      />
  ...
  <basemes
    ...
    "
    :style="
      $store.state.md_trademark.status_trademark == 0
        ? { background: 'rgb(118, 226, 118)' }
        : $store.state.md_trademark.status_trademark == 1
        ? { background: 'yellow' }
        : { background: 'red' }
    "
  >
...
export default {
  ...
  setup() {
    ...
    const updates = (data) => {
      store.state.md_trademark.data.name = data.name;
      store.state.md_trademark.data.img = data.img;
      store.state.md_trademark.data.id = data.id;
      store.state.md_trademark.data.status = true;
    };
    return {
      data,
      deletes,
