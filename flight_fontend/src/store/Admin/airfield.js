...
const airfield = {
  ...
  actions: {
    ...
    //post
    async post_airfield({ commit, state }) {
      console.log(state.data);
      await axios
        .post("http://localhost:8081/admin/airfield", state.data)
        .then((e) => {
          state.mess_airfield = e.data.mess;
          state.status_airfield = e.data.status;
        });
      await axios
        .get("http://localhost:8081/admin/airfield", {
