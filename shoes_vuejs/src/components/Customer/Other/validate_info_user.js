...
export default () => {
  // regex validate
  const regex_name = /^[A-Za-z][A-Za-z0-9_]{7,29}$/;
  console.log(regex_name);
  const regex_phone = /^[0-9\-+]{9,15}$/;
  const regex_email =
    /^[a-zA-Z0-9.!#$%&’*+/=?^_`{|}~-]+@[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*$/;
  const regex_passport = /^(?!^0+$)[a-zA-Z0-9]{3,20}$/;
  ...
  const check_passport = () => {
    if (
      ...
      store.state.err2.passport = "Hộ khẩu không được bỏ trống";
    } else if (regex_passport.test(store.state.dataf2.passport) == false) {
      store.state.err2.passport = "Hộ khẩu Không hợp lệ";
    } else {
  ...
  const nextpayment = () => {
    if (store.state.list.my_ticker.id_tick.length == 1) {
      if (store.state.list.my_ticker.id_seat.length < 1) {
        console.log("Ban chua chon ghe 1");
      } else {
        aplyy();
        router.push({
          name: "payment",
      ...
      if (store.state.list.my_ticker.id_seat.length < 2) {
        console.log("Ban chua chon ghe 2");
      } else {
        aplyy();
        router.push({
          name: "payment",
