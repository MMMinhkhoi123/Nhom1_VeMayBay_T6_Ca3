...
export default {
  ...
  setup() {
    ...
    const submit_delete = () => {
      ...
      setTimeout(() => {
        if (store.state.md_location.mess_location == null) {
          const storage = getStorage();
          const desertRef = ref(storage, "location/" + data_remove.img);
          deleteObject(desertRef);
          store.state.md_location.mess_location = "delete location success !";
        }
      }, 200);
    };
